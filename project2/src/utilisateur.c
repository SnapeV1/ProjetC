...
FILE * f=fopen(filename, "r");
    ...
    {
u.cin=0;
        while((fscanf(f,"%d %s %s %d %d %d %s %s %d %d",&u.cin,u.nom,u.prenom,&u.DateNaissance.annee,&u.DateNaissance.mois,&u.DateNaissance.jours,u.genre,u.type,&u.numBV,&u.vote)!=EOF)&&test==0)
        {
            if(u.cin==id)
                test=1;
        }
...
else if((strcmp(u.genre,"femme")==0)&&(u.vote!=-1)&&(strcmp(u.type,"Electeur")==0))*f=*f+1;
